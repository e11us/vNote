...
public class mySQLportal {
	...
	private static void connectTo() {
		if( outputMsg )
			p.p( "data.mySQLportal", "connectTo()" );
		try{
			Class.forName( "com.mysql.jdbc.Driver" );
			//
			String userName= _SysConfig.getSQL_LI()[1];
			String password= _SysConfig.getSQL_LI()[2];
			String url= _SysConfig.getSQL_LI()[0];
			//
			con= DriverManager.getConnection( url, userName, password );
			p.p( "data.mySQLportal", "connection established successfully." );
	...
	private static void createESMtable() {
		if( outputMsg )
			p.p( "data.mySQLportal", "createESMtable()" );
		//
		...
		try{
			...
			stmt.close();
			if( outputMsg )
				p.p( "data.mySQLportal", "master table is refreshed." );
		}catch ( SQLException e ){
			e.printStackTrace();
		}
		//
	...
	public static void refreshAllCol() {
		p.p( "data.mySQLportal", "refreshAllCol()" );
		//
	...
	private static void ExecUpdate( String inp ) throws SQLException {
		connect();
		if( outputMsg )
			p.p( "data.mySQLportal", "ExecUpdate: " + inp );
		//
		Statement stmt= null;
		stmt= con.createStatement();
	...
	private static ResultSet ExecQuery( String inp ) {
		connect();
		if( outputMsg )
			p.p( "data.mySQLportal", "ExecQuery: " + inp );
		//
		try{
